...
const VariableProximity = forwardRef<HTMLSpanElement, VariableProximityProps>((props, ref) => {
  ...
  useAnimationFrame(() => {
    ...
    letterRefs.current.forEach((letterRef, index) => {
      ...
      if (distance >= radius) {
        // Use font-weight if wght axis exists, otherwise use fontVariationSettings
        const wghtAxis = parsedSettings.find(s => s.axis === 'wght');
        if (wghtAxis) {
          letterRef.style.fontWeight = wghtAxis.fromValue.toString();
        } else {
      ...
      const wghtAxis = parsedSettings.find(s => s.axis === 'wght');
      if (wghtAxis) {
        const interpolatedWeight = wghtAxis.fromValue + (wghtAxis.toValue - wghtAxis.fromValue) * falloffValue;
